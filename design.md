...
> 当前版本: **v1.20**　状态: **M0 ~ M6 全部交付 + post-M6 UX 迭代 wave 1+2 + 简易用户登录模块 (v1.17) + Release 流程 canonical 文档 + bootstrap.sh (v1.18) + target_versions registry (v1.19) + run.errored 列 + error verdict + 进度条 + case 隔离 (RESET ALL + prepare_threshold=None) + 时区 root fix + admin Bearer-token 迁移 + observability (step_id sanitize / .error.txt) + hadoop_simple external (v1.20)**（详 §0 历史表 + §13.17）。
...
- `smoke-runner` 在 PR **merge 后**由 foreman 派(background);跑 `scripts/smoke.sh` 用 known-good case 验 harness 工具链在真集群健康;NO-GO → foreman 核对清单后开 revert PR(§15.1 step 6.a)。
...
5. **foreman** 见到"该 PR merged" → **派 smoke-runner**（background，merge 后验收）→ GO 标该项完成；NO-GO → 核对 squash 清单后开 revert PR + escalate。回 step 2。
...
|---|---|---|---|
| lg-bug-0001 hashjoin 右表 | open | PASS | upstream-fixed（候选改 status: fixed；EXPLAIN plan 含 tmp_test02 = 优化器选对小表） |
| lg-bug-0002 unnest crash | open | PASS | upstream-fixed（temp table 路径未触发 recover mode + log_grep matches=0） |
| lg-bug-0003 count-no-statistics | open | PASS | upstream-fixed（ANALYZE 后 NOTICE 流不含 "do not have statistics"） |
| lg-bug-0004 CTAS rowcount=0 | open | PASS | upstream-fixed（ORCA off + clock_timestamp + REPLICATED 不再 0 行） |
| lg-bug-0005 LC_CTYPE upper | open | PASS | upstream-fixed（LC_CTYPE='C' 数据库里 upper(multibyte) 不报错） |
...
|---|---|---|---|---|
| lg-bug-0001 hashjoin 右表 | open | CLI PASS | **API PASS 2204ms**（真 EXPLAIN，含小表 tmp_test02 选对）| upstream-fixed |
| lg-bug-0002 unnest crash | open | CLI PASS | **API PASS 104ms**（unnest 不 crash + recover mode log 0 matches）| upstream-fixed |
| lg-bug-0003 count-no-statistics | open | CLI PASS | **API PASS 60ms**（ANALYZE 后 NOTICE 流不含 "do not have statistics"）| upstream-fixed |
| lg-bug-0004 CTAS rowcount=0 | open | CLI PASS | **API PASS 44ms**（CTAS rowcount > 0）| upstream-fixed |
| lg-bug-0005 LC_CTYPE upper | open | CLI PASS | **API PASS 5467ms**（LC_CTYPE='C' DB 里 upper(multibyte) 不报错）| upstream-fixed |
...
**R28 (intermittent sampling) 满足**：M4c-1 case 3 轮 real-cluster verification（Run #15 PASS / #16 PASS / #18 SKIP-as-expected），全部一致。external_systems case 依赖外部 ES 服务，不存在 lg-bug-0009 那种"3 rounds 假阳性"风险。R28 ≥3 rounds 底线已踩到。
...
4. 收回结果，evaluate honestly：
   ...
   - 任何歧义当未完成。
5. 决定下一步：
   - 通过（reviewer APPROVE + CI SUCCESS + merged）→ **派 smoke-runner（background，merge 后验收，review-pipeline v3）** → **GO** 才标该项 done；**NO-GO** → `git show <squash-sha> --stat` 核对文件清单 == 该 PR 预期范围（§5.5 实测约束：squash 混了无关文件会被全删）→ 干净则 `git revert` 开 revert PR，不干净则 escalate。写 state.json `smoke_verdict`；回 step 1。
