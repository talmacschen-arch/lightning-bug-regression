...
### 16.4 external_systems — 依赖外部服务组件的集成测试（v1.10 新增）
...
**关键差异 (vs extension)**：
- `extension` 假设 `CREATE EXTENSION foo` 即用，依赖 PG 二进制里已有 `.so`（部署时就绪）
- `external_systems` 需要**外部服务进程 + 凭据 + 网络 + profile.d**全部就绪——`datalake_fdw` / `hive_connector` / `PXF` / `zombodb` 这类典型；环境未就绪时 status=`awaiting_env`（不是 fail，是 lifecycle 状态）
...
**`status: awaiting_env` 的 lifecycle 语义**：
- 新 case 蓝图写好（YAML 完整 + `external_deps` 列清）但目标集群未部署该外部服务时，case status 标 `awaiting_env`（"等环境就绪"）
- 环境就绪后 user 手动改 stable
- M5 / M6 后 runner 真消费 `external_deps` 后才能自动验证
