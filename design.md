...
> 当前版本: **v1.27**　状态: **M0 ~ M7 全部交付 + post-M6 UX 迭代 wave 1+2 + 简易用户登录模块 (v1.17) + Release 流程 canonical 文档 + bootstrap.sh (v1.18) + target_versions registry (v1.19) + run.errored / error verdict / 进度条 / case 隔离 / 时区 root fix / admin Bearer 迁移 / observability / hadoop_simple external (v1.20) + external_systems status 双轴 (v1.21) + review 流水线补齐并端到端跑通 (v1.22~v1.24) + M7 LLM 草稿生成交付 (v1.25) + case id 前缀规范化 + Runs 详情失败可见性 (v1.26) + status 漂移对账 + 用例扩充 bug-0013~0024 + 文档脱敏 (v1.27, released v0.27.0)**（详 §0 历史表 + §13.17）。
...
| 版本 | 日期 | 作者 | 变更摘要 |
|------|------|------|----------|
| v1.27 | 2026-08-13 | Claude | **status 漂移对账功能 + 回归用例扩充（bug-0013~0024 等）+ 文档脱敏；release v0.27.0。** 自 v0.26.0（v1.26）以来 27 个实质提交（另 140 条 cron report rollup 不计）。(1) **status 漂移对账**：`status`（手工元数据轴）与 runs.db 每次 verdict（判定轴）会随时间漂移（BUG 早修好跑绿但 YAML 还停 `open`）——新增只读对账，不自动改 YAML。后端 `GET /cases/status-drift` + 单一真源 `backend/app/utils/status_drift.py`（classify 不许两处实现，铁律5），Dashboard 只读卡片（PR #219）；`scripts/status-drift.py` CLI 复用同一份逻辑，REGRESSION→exit 2 可当 CI informational gate；build 387 一次性对齐 7 个 stale 标签（PR #218）。**已知语义**：drift 工具是 build/DUT 无关的——同一 case 在带修复的 DUT（如 Lightning 2.5.3）跑绿、在另一产品线旧 build（如 SynxDB4 4.5.3-2 build 31）跑红时会报"假回归 REGRESSION"，需人对照 target 判断，`fixed` 标签不因此翻转。(2) **回归用例扩充**：新增 `bug-0013` ao_column 分区 reindex-invalid（#205）/ `bug-0014` PAX count(*) Index Only Scan hang（#207，#209 mark fixed）/ `bug-0015` pg_dump 不支持 PAX 表导出（#208）/ `bug-0016` gppkg --clean SyncPackages.ret AttributeError（#211）/ `bug-0017` leaf delete+vacuum 后 ANALYZE ROOTPARTITION crash（#214）/ `bug-0018` ORCA force_split_window TPC-DS q44 crash（#215）/ `bug-0019` LC_COLLATE/CTYPE='C' + ORCA 多字节列别名表头 UNKNOWN（#216）/ `bug-0020` 关联子查询 UNNEST() PG planner PlaceHolderVar 结果错（#217）/ `bug-0021` plpgsql EXCEPTION 块 optimizer=off segworker-groups（#221）/ `bug-0022` ORCA 1500 数组列投影 hang（#222）/ `bug-0023` ORCA UNION ALL band-join 计划爆炸 hang（#223）/ `bug-0024` PAX BTREE 索引扫描比 ao_column 慢一倍（#230）；`ext-plr-dplyr-basic`（#212）/ `ext-madlib-linregr-naive-bayes`（#213）；`xs-datalake-fdw` HDFS Snappy Parquet（#224）；`xs-zombodb-mixed-predicate` 分区错误结果 case + 3 扫描计划加固 + status open→fixed（#220/#225~#229）；`ext-anon` role drop 对 start_dynamic_masking grants 健壮化（#210）。(3) **前端**：Cases 页新增 status 筛选（#206）。(4) **文档脱敏**：cases 移除品牌/来源标识改中性叙述（bf30d9d）。 |
| v1.26 | 2026-06-02 | Claude | **case id 前缀规范化 + Runs 详情失败可见性 + 文档脱敏/收尾。** (1) **case id 前缀统一为 `bug-` / `ext-` / `xs-`**：19 个 case 文件重命名 + `id` 字段 + notes 交叉引用，`case_categories.id_prefix` 经 alembic **0007** 同步更新（已 apply 的 0001/0002 不动），skill 规则 / 测试 fixture 对齐（PR #200）。**部署提示**：含此类 seed 迁移的改动 merge 后须对长期运行的 dev DB 跑 `alembic upgrade head`，否则 `yaml_loader.py` 前缀强校验会报错（实战暴露）。(2) **Runs 详情页失败可见性**：fail/error case 行高亮（红/橙底 + 左色条）+ status badge 配色（PR #202）；新增 "Only failed/errored" 筛选开关（PR #203）；色板抽到 `frontend/src/lib/caseStatus.ts` 两页复用。(3) **文档收尾**：移除孤立 release-notes 文件 + 头部/README 的 release 死链（PR #201，配合手删旧 GitHub Release+tag）；全仓 md 文档移除内部代号字样（保留产品出处/真实链接由后续按需处理）。 |
...
文件路径：`cases/<id>.yaml`（id 用 kebab-case，例如 `bug-0042-vacuum-deadlock.yaml`）。
...
```yaml
# —— 标识与元信息 ——
id: bug-0001-hashjoin-right-table   # 全局唯一，与文件名一致；英文 kebab-case
                                       # 命名：bug → bug-NNNN-<slug>；extension → ext-<extname>-<slug>
...
**§14 R 编号**：本约定 forensic 来自 PR #18 (commit `b49b766`) + bug-0005 在 dogfood 上的 ERROR 实测；defense-in-depth 的 sql_driver autocommit 实现是 F-3 的副产物。本身是正向 prescription 不是反模式，所以**不**新加 R 条目；reviewer 在 cases YAML 审查时如发现 non-tx-safe DDL 没走 `psql -c` 应直接 REQUEST_CHANGES 引用本节。
...
CREATE TABLE case_skip_list (
  id INTEGER PRIMARY KEY,
  case_id TEXT NOT NULL,             -- YAML id；可用前缀匹配（例如 'bug-0003'）
...
CREATE TABLE case_categories (
  ...
  id_prefix TEXT NOT NULL UNIQUE,        -- case id 必须以此开头："bug-" / "ext-"
...
INSERT INTO case_categories (name, display_name, description, id_prefix, dir_path, status_whitelist, default_status, display_order, created_by) VALUES
  ('bug_regression',
   'BUG 回归',
   '历史 BUG 的复现 / 修复验证用例。来源主要是飞书历史 BUG 文档。',
   'bug-',
   'bug-regression',
...
6. **id 前缀与 category 匹配**：`bug_regression` 必须 `bug-*`；`extension` 必须 `ext-*`。前缀错 = skill bug，立即修正。
...
lightning-bug-regression/
  ...
  cases/                       # 用例 YAML，PR review 入口
    bug-regression/            # v0.7：category=bug_regression 的用例
      bug-0001-hashjoin-right-table.yaml
      bug-0002-array-unnest-crash.yaml
      ...
    extension/                 # v0.7：category=extension 的用例
      ext-pgvector-basic.yaml
      ext-postgis-spatial.yaml
      ext-pgcrypto-hash.yaml
      ...
...
- **M4c external_systems 首批 case** — 1 例 done (xs-zombodb-partition-text-search, PR #104, status=stable, 9/9 PASS dogfood，M6-5 落地后改 Jinja 渲染)；后续 2 例由用户按环境就绪节奏添加（datalake_fdw / hive_connector / PXF 各候选），蓝图可先写 `status: awaiting_env`；详 §13.10
...
5. 第一批用例落 `cases/bug-regression/`（全部 `category: bug_regression`、`status: open`，9.3 因无可执行 SQL 用 `status: stub`），用 **v0.5 step 级 `expect:` 写法**：
   - `bug-0001-hashjoin-right-table.yaml` (9.1, `status: open`)
   - `bug-0002-array-unnest-crash.yaml` (9.2, `status: open`)
   - `bug-0003-count-no-statistics.yaml` (9.3, `status: stub`)
   - `bug-0004-ctas-rowcount-zero.yaml` (9.4, `status: open`)
   - `bug-0005-lc-ctype-upper.yaml` (9.5, `status: open`)
...
- **M3a-10 dogfood**: 真起 backend + frontend，**人类用本 skill / 手写一份新 case YAML**（例：`bug-0006-test-m3a-flow` stub），走 Validate → Try → Save 一遍，看 PR 真开出来 + auto-merge fire。产 `docs/m3a-dogfood-<ts>.md` 报告
...
**完成定义**：
- 1~3 例 xs-* YAML 落在 `cases/external-systems/`，validate 全 ok
...
- **M7-2 prompt 设计 + few-shot examples**
  - 抽 3 个 representative cases 作 few-shot（hardcode 在 prompt module，**不**做 dynamic /admin/few-shot endpoint——过度抽象）:
    1. `bug-0001-hashjoin-right-table` (BUG, 简单 GUC + SELECT 验证)
    2. `ext-pgvector-ivfflat-basic` (extension, CREATE EXTENSION + INDEX + plan_contains)
    3. `bug-0008-pax-toast-vacuum-analyze-crash` (跨 driver kind: sql + kind: shell psql -c，§4.1.2 实战)
...
**🟠 R28：intermittent BUG 采样不足导致假阴性（v1.10 新增，M4a bug-0009 反转 fixed→open 实战）**
...
- 后果（bug-0009 §9.12 UNION ALL 行序错误，2026-05-24 实战）：V1 PR 用 3 rounds repetition 跑 → 本地 + dogfood + CI 全 PASS → 落 main + `status: fixed`。用户 review 立刻喊 "3 次会误判，改成 10 次"。改 10 rounds → round 1 立刻 fail → 反转 `status: open` + `fixed_version: ""` + notes 加 "Status reverted" 段落。第一例 user catch 比 reviewer / CI / dogfood 都早的案例。
- 正确做法：
...
- 来源：bug-0009 §9.12 V1 PR + V2 user-driven fix（PR #79 commit chain），user 主动 catch 比所有自动化都早；§13.9.5 详细分析。
...
- 正确做法：本项目用 `bug-NNNN-<语义描述>` 命名（已采用），避免章节号；目录按语义分类（如 `cases/optimizer/`、`cases/storage/`）。
...
CREATE TABLE case_categories (
  ...
  id_prefix TEXT NOT NULL UNIQUE,        -- case id 必须以此开头：bug- / ext- / xs-
...
- 原 5 例 (PR #47 status 改 fixed)：bug-0001 hashjoin / 0002 unnest / 0003 ANALYZE / 0004 CTAS / 0005 LC_CTYPE upper
- M4a 飞书 3 例：bug-0007 §9.7 orca-sort (#70 fixed) + bug-0008 §9.11 pax-toast (#74/#80 fixed) + bug-0009 §9.12 union-all (#79 **open**，§14 R28 intermittent 实战)
- M3a dogfood 副产 1 例：bug-0006-m3a-dogfood-smoke
...
**id_prefix `xs-` 决策**（PR #88，user-approved 2026-05-24）：
- `ext-sys-` 与 `ext-` startswith 冲突（虽然 backend 用 cat.id_prefix 查表不用 startswith，但语义上有歧义）→ 否决
- `es-` 与 Elasticsearch 缩写冲突 → 否决
- **选 `xs-`**：4 char，xs = external systems 缩写，无任何冲突
...
**v1.21 反转原因**（2026-05-26）：v1.10 设计时 status_whitelist 只覆盖环境就绪度 (stable / awaiting_env)，与项目主目的"BUG 回归测试"语义不符——external_systems 与 extension 拆分的真实理由是依赖外部进程，不是 case 性质不同。3 个已落库 case (`xs-pxf-hdfs-order-by-writable` / `xs-pxf-hive-fdw-encoding-utf8` / `xs-zombodb-partition-text-search`) 都是 BUG 复现性质，用 `stable` 无法区分"BUG 已修复 vs BUG 未修复但环境跑得起来"。
...
```yaml
id: bug-NNNN-<slug>                  # 或 ext-<extname>-<slug> 或 xs-<svc>-<slug>
title: <中文 OK>
...
6. **id 前缀 × category 一致性** — bug→`bug-*`、ext→`ext-*`、external_systems→`xs-*`
...
**M3b sprint**（详 §13.8 plan，已 done）：10 子步骤完成 + 1 bug 修补（assertions list-form fix, PR #67 dogfood 暴露）+ M3b-10 web flow dogfood 闭环（PR #68 ext-pgvector via skill 路径真 merge 进 main）。SKILL.md 588 行 bundle 在 PR #65 内交付。
...
- **关键 PR**：#63（cases?q=）+ #64（/admin/step-kinds）+ #65（SKILL.md 588 行 bundle）+ #66（lint）+ #67（assertions list-form fix）+ #68（首例 ext-pgvector via skill 路径）+ #69 dogfood 报告 / 硬化：#85 model pin + #86 7 spec fixes + #89 追问迁 external_systems
...
- **M4a 飞书 BUG 补录**：3 例新 case (`bug-0007` §9.7 / `0008` §9.11 V1+V2 / `0009` §9.12 intermittent) + sql_driver chain refactor（PR #73→#75→#80 autocommit→commit-after-step）
- **M4b extension 首批 5 例**：pgvector #68 / pg_partman #82 / anon #83 / plpython3u+numpy #84 / postgis #87 — **§12 首批 3~5 例上限达成**
- **核心副产**：§14 R28 入档（intermittent BUG sampling ≥10 rounds，`bug-0009` 反转 fixed→open 实战）；§16 测试门类 super-section（PR-A1，本 reorg 链 §16）；§17 skill super-section（PR-A2，本 reorg 链 §17）
- **完成定义**：3 类 case YAML 共 14 例 stable / 5 旧 BUG `fixed` 维持 / `bug-0009` `open`（铁律 1 实战）
...
- **完成定义**：1~3 例 `xs-*` YAML 落 `cases/external-systems/`，validate ok；前端 tab 列出
...
| M4 | R28 (intermittent BUG sampling ≥10 rounds) | sql_driver chain refactor + bug-0009 反转 fixed→open 铁律 1 实战；§4.1.2 psql -c 约定硬化 |
