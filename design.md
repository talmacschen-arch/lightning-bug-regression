...
> 当前版本: **v1.3**　状态: **A/B/D 自检全 ✅。B 戳破 CronCreate 假设改 OS crontab + claude --print；A workflow scope 已补（2026-05-23 实测 x-oauth-scopes: repo, workflow）；D 全过。M0 step 1 可开**。
...
| Settings → General → Features | Issues / Discussions / Wiki / Projects | 全开（v0.4 Q12） |

**关键澄清**：agent ≠ GitHub 角色。§8 的 7 个 agent（pm-designer / scheduler / backend-fixer / frontend-fixer / doc-writer / reviewer / smoke-runner）**只**在本机 Claude Code 里跑，**不**对应 GitHub PR 上的 Reviewer / Assignee / Author 字段。`reviewer` agent 跑完输出 "APPROVE / REQUEST_CHANGES" 是给你本人看的决策辅助，**不会也无法**在 GitHub PR 里点 Approve 按钮。所以"单账号 author + merger"不会被 review 流程卡住。
...
| **reporter**（v1.0 新增） | haiku | ✅（仅写 docs/status/） | **OS crontab 12:00 / 00:00 + `claude --print "/report-status"`**（v1.3 改路径），独立 OS 进程 | `docs/status/YYYY-MM-DD-HHMM.md`（v1.2：飞书推送已移除，**人工查目录**）；运转规则详 §15.3 |
...
7. **reporter** 在 12:00 / 00:00 fire，读 state.json + git log + gh pr list 出报告。
...
1. **`scheduler` 一次派 ≥ 2 个 agent 时，每个必须带 `isolation: "worktree"`**。Claude Code 会为每个 agent 自动开一个临时 git worktree（独立 branch），互不串数据。
...
lightning-bug-regression/
  ...
  docs/
    status/                    # v1.0：foreman state + 定时报告
      ...
      2026-05-24-0000.md       # reporter cron 00:00 输出
      ...
...
| Q12 | ✅ v0.4 | GitHub 仓库 features 启用范围 | **全开**：Issues / Discussions / Wiki / Projects。理由：用户要求"能启用的都启用"，不预先做减法。 |
...
| Q20 | ✅ v0.5 | 多 agent 并发的隔离 | **强制 `isolation: "worktree"`**，且 `scheduler` 在派活前跑 `.claude/scripts/check_agent_dispatch.sh` 校验。理由：preflight 2026-05-18 事故——共享 worktree 下 lint agent 把另一个 agent 的 WIP 一起 commit，CI 全红。 |
...
| Q28 | ✅ v1.0 | 多 agent 协作能不能形成 loop / 不用人盯 | **能**。§15 落地三件套：(a) foreman verify loop（10 round / 2h budget，stop conditions 明确）；(b) GitHub auto-merge（specialist 设 `gh pr merge --auto --squash` 后即退，CI 全绿自动合并）；(c) cron 12:00 / 00:00 定时 reporter 汇报。preflight `.claude/agents/foreman.md` 已生产验证 5 周（2026-04~05）。 |
| Q29 | ✅ v1.0（v1.2 改） | 12:00 / 00:00 报告输出到哪儿 | **仅本地 `docs/status/<YYYY-MM-DD-HHMM>.md`，用户人工到目录下查**（v1.2 用户决策：飞书 chat 未打通）。报告 schema 固定 8 段不变，**§4 needs_human 是唯一允许"挂起"的事项类**。原 v1.0 的飞书推送链路已删除，对应 `feishu_send_*` 代码 / `feishu_report_chat_id` 设置项都不实现。 |
...
0 0  * * * cd /data0/chenqiang/project009/lightning-bug-regression && /root/.local/bin/claude --print "/report-status" >> docs/status/cron.log 2>&1
# 验证：
crontab -l   # 看到两行
date         # 等到下个 12:00 / 00:00 后看 docs/status/ 有新 md
```
...
8. **注册 OS crontab**（M0 收尾的最后一步，**用户亲手执行一次**；v1.3 改路径）：root 跑 `crontab -e`，加两行（详 §15.3.1）：
   ...
   0 0  * * * cd <repo-abs-path> && /root/.local/bin/claude --print "/report-status" >> docs/status/cron.log 2>&1
   ```
...
- 触发：scheduler 一次派 2 个 fixer，都在主 worktree 上动文件。
...
本章解决一个问题：**foreman 一启动，人就不用管了——开发自循环、PR 自动合并、12:00 和 00:00 自动汇报，需要人决策的事情统一在报告里列出。**
...
# 验证：
crontab -l   # 看到两行
# 等到下个 12:00 / 00:00 后查 docs/status/ 有新 md + docs/status/cron.log 有调用日志
```

**每次 fire 的执行模型**（OS crontab + `claude --print`）：
1. OS cron 在 12:00 / 00:00 启动一个**全新的 claude 进程**——独立于任何在跑的 foreman session
...
reporter 不改代码，不调度 specialist，不处理决策，**不发外部消息**——它**只是把状态写进目录**，你 12:00 / 00:00 之后自己来看。
...
9. **第一次启动 foreman**：你跑 `/foreman M1`，给 sprint 标签；foreman 接管 M1 全部子任务；12:00 / 00:00 你看报告。
...
- 一个仪表盘（reporter + 12:00/00:00 cron）
- 一组保险丝（§15.4 防护表）
