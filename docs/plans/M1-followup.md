...
- [x] **F-2 sql_driver 把 EXPLAIN 输出 populate 到 StepResult.plan_text**（`backend/app/runner/sql_driver.py` + tests + `assertions.py` 调整）— PR #17 (c5a9d11)
  - 问题: M1-11 dogfood bug-0001 实跑 stdout 含 `Hash` + `tmp_test02`（BUG 已 upstream-fixed），但 `_plan_contains` 拿不到 `plan_text` 返 None 导致 case FAIL（假阳性）
  ...
  - 验收: bug-0001 重跑 PASS（BUG 已修，runner 正确识别）
  - § 引用: §4.1 (`expect.plan_contains`) + §5.3 sql_driver

- [x] **F-3 sql_driver 处理 non-tx-safe DDL**（`backend/app/runner/sql_driver.py` + tests）— PR #18 (b49b766) + 收尾 commit (psql -c convention 落地，bug-0005 改 `su - gpadmin -c "psql -c ..."` 形式，design.md §4.1 + dogfood normalizer 同步加 `psql ` 子串路由到 shell driver 的约定)。F-3 的 `_NON_TX_DDL_RE` 仍保留作 defense-in-depth（用户漏写 psql 前缀时 sql_driver 兜底自切 autocommit），但**首选写法**是 `su - gpadmin -c "psql -c '<DDL>'"`
  - 问题: M1-11 bug-0005 跑 `DROP DATABASE mydb` 报错 — psycopg 默认 `autocommit=False` 把 DDL 包进 tx，PG 拒绝 tx 里 DROP/CREATE DATABASE
  ...
  - 验收: bug-0005 重跑 PASS 或 FAIL（取决于实际 BUG 状态），不再 ERROR
  - § 引用: §5.3 sql_driver + §14 R9（异常折叠）

## 完成定义 ✅ done 2026-05-24

- F-1/2/3 全 [x] ✓
- 重跑 dogfood → **5/5 PASS**（`docs/m1-dogfood-2026-05-23-1828.md`）：bug-0001 hashjoin（F-2 plan_text 起效）/ bug-0002 unnest crash / bug-0003 count-no-stats / bug-0004 CTAS rowcount / **bug-0005 LC_CTYPE**（F-3 + psql -c routing + su gpadmin wrap 综合奏效）
- 3 个 needs_human 清掉
